...
// Returns 1 for consistent, 0 if inconsistent. Check that the arrays of positions (x,y,z) [0..N-1] are consistent, in the sense that pistons placed at the positions and having radius r are at least separation away from each other. Actually, I cheat. The geometry of measuring the distance between the edges of two pistons on the surface of a transducer is a little tricky, so all I do is ensure that the centres are more than 2r+separation from each other. If this is the case, then they are at least 2r+separation from each other when properly placed, i.e. noncoplanar. 
...
void Randomly_Move_Pistons_Normal(double *x, double *y, double *z, double r_sphere, int N,double amount){ // Shift each transducer centre by a random offset in both x and y,   normally distributed with a std deviation of 1, then multiplied by "amount" to get a distance. I use the Box-Muller transform to get normally distributed values.
	srand(time(NULL));
...
void Setup_Regular_Hexagonal_Straight_Lines(double *&x, double *&y, double *&z, double r_piston, double r_sphere, double separation, double r_aperture,double r_transducer,  int &N){ // TOO TRICKY FOR NOW! Fill up the transducer with hexagonally packed pistons by placing a line of pistons across the y=0 axis, making sure the minimum spacing between elements is separation, they don't overlap the external radius which is r_transducer, and they don't overlap the internal radius, which is r_aperture.  Output number of elements to N.  
	...
	//NOTE !!!!!!!!!!!!!!!!!!!!!!!!!!!
	// If you overwrite N, you better resize x,y,z otherwise you will get a seg fault.
...
void Setup_Regular_Dumb_Packing_Straight_Lines(double *&x, double *&y, double *&z, double r_piston, double r_sphere, double separation, double r_aperture,double r_transducer,  int &N){ //   Fill up the transducer with pistons by placing a line of pistons across the y=0 axis, making sure the minimum spacing between elements is separation, they don't overlap the external radius which is r_transducer, and they don't overlap the internal radius, which is r_aperture.  Output number of elements to N.  
	// ACTUALLY!!! This is not quite right. What I'm doing here is placing points regularly on a square and then projecting back to the sphere. It's not great, but it'll do. 
	// So this is how we do it. The separation between centres is s = 2r_piston+separation. Start at ind_x = -rtranscucer/s and ind_y =  -rtranscucer/s. Increment them and test to see whether the relevant point makes it into the sphere.
	int Ntemp;
...
void Circular_Piston_Transducer::Setup_Transducer(string &filename) // Grab information from the file and use it to set up the transducer.
{ /* filename is the name of the file with the transducer parameters in it. This should contain, in order:
   1) 5 doubles: Radius of curvature of the transducer, radius of piston, minimum separation, radius of transducer in projection, radius of interior hole in transducer.  
   ...
   6) IF option is appropriate: 4 double: one line for each transducer element,  x,y,pressure_real,pressure_imag. Of course these pressures might not be used, but load them in anyway.
  */
   ifstream Parameters; 
...
{
      cout << "2 Couldn't open your parameter file. Sorry. No success this time. " << filename << endl;      
}     
   ...
   getline(Parameters,discardMe); // go to next line, discarding all comments on rest of line.
#ifdef DEBUG
	...
	Input_Number_Of_Elements(li_number_of_elements) ; // NOTE must call this otherwise you never assign memory to the different elements and get SIGSEGVs. 
   ...
   cd_omega_div_c = (PI2*ld_freq/ld_speed); 
	...
	// OK, now it's time to break into different cases. But note: all the different cases do is give different positions of the transducer elements. The actual generation of things can come later and will be the same for all cases. So do it like that! Don't use constructors for this - you might want to setup transducers multiple times and you don't want to have to quit the program to do that.
...
//	std::cout << "Test succeeded " << std::endl;
	...
	for (int li_ind = 0; li_ind < ci_number_of_elements ; li_ind++) { // OK, I could use function calls for these - I implemented the functions in a previous version - but I don't see what the gain would be.
   ...
   this->Generate_All_Elements(); // Now all of the transducer elements set themselves up. i.e. they assign info to their computational elements. 
   Parameters.close();
}
